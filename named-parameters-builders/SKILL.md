...
4. If parameters are just enum dispatch, collapse the enum into named methods when this reduces ambiguity.
...
- Prefer `nord.execute_admin_kind()` over `nord.execute_kind(ActionKind::Admin)` when the enum only selects the method body.
...
- enum-to-method collapse did not duplicate divergent logic
